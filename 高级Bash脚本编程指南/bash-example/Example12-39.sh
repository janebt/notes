...
#  使用rsync 命令从镜像站点上下载Fedora 4 的更新. 
#  为了节省空间, 如果有多个版本存在的话, 
#+ 只下载最新的包. 
...
init () { 
# 让管道命令返回可能的rsync 错误, 比如, 网络延时(stalled network). 
set -o pipefail 
...
if [ -z "$pkg_name" ]; then #  如果不是一个rpm 文件, 
