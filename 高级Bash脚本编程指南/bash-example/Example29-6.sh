...
#  注意$LOGFILE 必须是可读的
#+ (用root来做：chmod 644 /var/log/messages). 
TEMPFILE=temp.$$ 
#  创建一个"唯一的"临时文件名, 使用脚本的进程ID. 
#  用'mktemp' 是另一个可行的办法. 
#  举例: 
